...
>
const mockUseGetWatchlistMoviesQuery =
  useGetWatchlistMoviesQuery as jest.MockedFunction<
    typeof useGetWatchlistMoviesQuery
  >
...
describe('Watchlist useContainer hook', () => {
  ...
  let triggerAddToWatchlist: jest.Mock
  ...
  beforeEach(() => {
    ...
    triggerAddToWatchlist = jest.fn().mockReturnValue({ unwrap: unwrapSuccess })
  ...
  it('should confirm and delete movie successfully', async () => {
    ...
    expect(args.title).toBe('Do you want to delete movie from watchlist?')
    ...
    expect(triggerAddToWatchlist).toHaveBeenCalledWith({
      inWatchlist: false,
      movieId: 1234,
    })
  ...
  it('should handle error when delete fails', async () => {
    ...
    triggerAddToWatchlist.mockReturnValue({ unwrap: unwrapReject })
